<template>
  <div class="container">
    <userAccounWithdraw :data="{todayMax:data.todayMax,todayRecord:data.todayRecord,info:data.withdrawInfo}"
                        @change="inputChange">
      <div class="btn-box">
        <van-button type="primary"
                    size="large"
                    @click="subWithdraw"
                    :disabled="(formData.num==0)||(data.todayRecord-data.todayMax>0)">提现</van-button>
      </div>
    </userAccounWithdraw>
    <!-- <div class="his-list">
...
<script>
import { getAccountInfo, subWithdraw } from '@/api/api.account.js'
import userAccounWithdraw from '@/components/userCenter/userAccounWithdraw'
...
export default {
  ...
  methods: {
    ...
    inputChange: function (e) {
      let _this = this
      _this.formData.num = e.value
    },
    subWithdraw: function () {
      let _this = this
      if ((_this.formData.num > 0) && (_this.data.todayMax - _this.data.todayRecord > 0)) {
        subWithdraw(_this.formData).then(res => {
          let icon = 'none'
          if (res.code === 200) {
            icon = 'success'
          }
    ...
    getPageData: function (isReset) {
      ...
      if (!_this.isend) {
        ...
        getAccountInfo(_this.pageconfig).then(res => {
          ...
          _this.data.todayMax = res.todayMax
          _this.data.todayRecord = res.todayRecord
          _this.data.withdrawInfo = res.withdrawInfo
          if (res.array) {
  ...
  onLoad (options) {
    if (options.type) {
      ...
      this.formData.type = this.pageconfig.type
      // 设置标题
